...
def train_trada_RF(N, steps, fold, random_state, sample_size, x_train, y_train, target_features, target_target):
    global R2_aoc,save_
    model = TwoStageTrAdaBoostR2(RandomForestRegressor(n_estimators=284,
                                                        max_depth=13,
                                                        min_samples_split=3,
                                                        min_samples_leaf=3,
                                                        max_features=0.5,
    ...
    if target_r2 > R2_aoc and save_:
        print('R2_aoc', R2_aoc)
        joblib.dump(model, 'best_tradaboost_rf_model.pkl')
        R2_aoc = target_r2
...
def objective_rf(trial):
    ...
    R2,_ = train_trada_RF(n_estimators, steps, 5, random_state, sample_size, x_train, y_train, target_features,
                        target_target)
    return R2
...
if __name__ == '__main__':
    ...
    best_model = joblib.load('best_tradaboost_rf_model.pkl')
    ...
    target_features.to_csv('F:/work_github/rf/rf_tl.csv', index=False, encoding='utf-8')